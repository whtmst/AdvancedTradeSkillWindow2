...
local function SelectCraftItem				(Index)
	if TradeSkill	then	return SelectTradeSkill(Index)
						elseif Index and Index > 0 then	return SelectCraft(Index)
						else	return end
end
...
local function GetPositionFromGame(Name)
	...
	local CraftCount = GetCraftCount()
	...
	until not GName or I >= CraftCount
...
function ATSW_ShowRecipe(Name)
	...
	if GamePosition and GamePosition > 0 then
...
function ATSW_ShowRecipeTooltip()
	...
	for I = 1, 19 do
		if I <= table.getn(Reagents) then
			...
			local Bank 				= ATSW_Options.ConsiderBank 	and ATSW_GetBankAmount(R.Name) 	or 0
			local Alts 				= ATSW_Options.ConsiderAlts 	and ATSW_GetAltsAmount(R.Name) 	or 0
			local Merchant 		= ''
